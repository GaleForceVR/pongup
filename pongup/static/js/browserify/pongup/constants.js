

// export const DATA_LOADED = 'DATA_LOADED';
export const HOME_ACTIVE = 'HOME_ACTIVE';
export const LADDERS_ACTIVE = 'LADDERS_ACTIVE';
export const LADDER_DATA_LOADED = "LADDER_DATA_LOADED";
export const LADDER_DETAIL_LOADED = "LADDER_DETAIL_LOADED";
export const LOGIN_ACTIVE = 'LOGIN_ACTIVE';
export const CREATE_USER = 'CREATE_USER';
export const SAVE_STATE = 'SAVE_STATE';
// export const INBOX_ACTIVE = 'INBOX_ACTIVE';
// export const INBOX_FAILURE = 'INBOX_FAILURE';
// export const LOADING_DATA = 'LOADING_DATA';
// export const MY_LISTINGS_ACTIVE = 'MY_LISTINGS_ACTIVE';
// export const MESSAGE_ACTIVE = 'MESSAGE_ACTIVE';
// export const TOGGLE_REPLY = 'TOGGLE_REPLY';
export const UPDATE_TAB = 'UPDATE_TAB';
export const UPDATE_LOCATION = 'UPDATE_LOCATION';
export const USER_DATA_LOADED = 'USER_DATA_LOADED';
export const MATCHES_DETAIL_LOADED = 'MATCHES_DETAIL_LOADED';
export const VALIDATE = 'VALIDATE';
export const INIT_EDIT_MODE = 'INIT_EDIT_MODE';
export const EXIT_EDIT_MODE = 'EXIT_EDIT_MODE';
export const UPDATE_SCORE = 'UPDATE_SCORE';
// export const UPDATE_UNREAD_ON_TAB = 'UPDATE_UNREAD_ON_TAB';
export const UPDATE_IS_IN_LADDER = 'UPDATE_IS_IN_LADDER';
export const SET_CURRENT_LADDER = 'SET_CURRENT_LADDER';
export const IS_MANAGER = 'IS_MANAGER';
export const IS_EDITING_RANKINGS = 'IS_EDITING_RANKINGS';
export const SET_NEW_RANKINGS = 'SET_NEW_RANKINGS';